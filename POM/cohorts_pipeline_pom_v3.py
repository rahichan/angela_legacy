...
def df_cleaning(df):
    df = df[df['Billing Country'] == 'DE']

    df = df.loc[:,['Name', 'Email', 'Financial Status', 'Created at', 'Subtotal', 'Shipping', 'Taxes', 'Total', 'Lineitem name', 'Billing Name', 'Id']]

    df.rename(columns={'Name':'Order','Financial Status':'Status' ,'Created at':'Creation_Date', 'Lineitem name':'Product Name', 'Billing Name': 'Customer_Name', 'Id':'Order_ID'}, inplace=True)

    df['Creation_Date'] = pd.to_datetime(df['Creation_Date'], errors='ignore', utc=True)
    ...
    first_orders = df.sort_values('Creation_Date').groupby('Email')['Order'].first().values
    ...
    df = df.astype({'Subtotal':'float64', 'Shipping':'float64', 'Taxes':'float64', 'Total':'float64'})
    df['ValueNOVAT'] = df.Total - df.Taxes
    return df
...
def cohorts_pipeline(df):
    ...
    trans_summary =  pd.DataFrame()
    trans_month = df.groupby(['Creation_Date_YM','Customer_Type']).agg({'Order_ID':pd.Series.nunique, 'ValueNOVAT':pd.Series.sum}).unstack(1).fillna(0).reset_index()
